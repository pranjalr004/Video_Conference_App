...
import { getCreateMeetingBreadCrumbs, getDashboardBreadCrumbs, getMeetingsBreadCrumbs, getMyMeetingsBreadsCrumbs, getOneonOneMeetingBreadCrumbs, getVideoConferenceBreadCrumbs } from '../utils/breadCrumbs'
...
function Header() {
  ...
  useEffect(() => {
    ...
    else if (pathname === "/mymeetings") setBreadCrumbs(getMyMeetingsBreadsCrumbs(navigate))
  ...
  const invertTheme = () => {
    ...
    localStorage.setItem("zoom-theme", theme === "light" ? "dark" :
      "light")
    ...
    },
    {
      items: [
        <EuiFlexGroup justifyContent='center' alignItems='center' direction='row' style={{ gap: '2vw' }}>
          ...
          <EuiFlexItem style={{ flexBasis: "fit-content" }}
            grow={false}>
            {
              isDarkTheme ? (
                <EuiButtonIcon
                  onClick={invertTheme} iconType="sun" display='fill' size='s' aria-label='theme-button-light' color='warning' />
              ) : (
                <EuiButtonIcon
                  onClick={invertTheme} iconType="moon" color="text" display='fill' size='s' aria-label='theme-button-dark' />
              )}
          </EuiFlexItem>
          <EuiFlexItem style={{ flexBasis: "fit-content" }} grow={false}>
